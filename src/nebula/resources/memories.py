...
class MemoriesResource:
    ...
    async def append(
        ...
        **For document engrams:**
        ...
        _raw = await self._core.request({
            ...
            "query": None,
            "body": body,
            "routing": {"header": "X-Nebula-Collection-Id", "body_fields": ["collection_id","collectionId"]},
            "idempotent": False,
        })
    ...
    async def create_upload(
        ...
        *,
        ...
        _raw = await self._core.request({
            ...
            "path_params": {},
            "query": {"filename": filename, "content_type": content_type, "file_size": file_size, "collection_id": collection_id, "client_idempotency_key": client_idempotency_key},
            "idempotent": False,
        })
        _raw = _raw["results"] if isinstance(_raw, dict) else getattr(_raw, "results", _raw)
        try:
    ...
    async def delete(
        ...
        _raw = await self._core.request({
            ...
            "routing": {"header": "X-Nebula-Collection-Id", "query_fields": ["collection_id","collectionId"]},
            "idempotent": False,
        })
        _raw = _raw["results"] if isinstance(_raw, dict) else getattr(_raw, "results", _raw)
        try:
    ...
    async def search(
        ...
        **Snapshot mode** (snapshot field): returns graph-search results with
        ...
        _raw = await self._core.request({
            ...
            "query": None,
            "body": body,
            "routing": {"header": "X-Nebula-Collection-Id", "body_fields": ["collection_ids","collectionIds","filters.collection_id","filters.collection_ids","search_settings.filters.collection_id","search_settings.filters.collection_ids"]},
            "idempotent": False,
        })
        _raw = _raw["results"] if isinstance(_raw, dict) else getattr(_raw, "results", _raw)
        try:
