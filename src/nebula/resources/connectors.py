...
class ConnectorsResource:
    ...
    async def disconnect_hosted_m365_tenant(
        self,
        workspace_id: str,
        *,
        request_options: Optional[RequestOptions] = None
    ) -> models.HostedM365TenantDisconnectResponse:
        """Disconnect a hosted Microsoft Entra tenant

        Disconnect the workspace from its hosted Microsoft 365 tenant, invalidate outstanding consent links, and require affected connections to reconnect. This removes only Nebula's local grant; it does not revoke tenant-wide admin consent in Microsoft Entra, which an Entra administrator must remove separately.

        operationId: connectors.disconnectHostedM365Tenant
        endpoint: DELETE /v1/connectors/m365/admin-consent
        """
        _raw = await self._core.request({
            "method": "DELETE",
        ...
        _raw = _raw["results"] if isinstance(_raw, dict) else getattr(_raw, "results", _raw)
        try:
            return models.HostedM365TenantDisconnectResponse.model_validate(_raw)
        except (ValidationError, ValueError):
            return _raw  # type: ignore[return-value]
