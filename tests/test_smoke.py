...
@pytest.mark.asyncio
async def test_single_ingestion_operation_calls_use_explicit_collection_routing() -> None:
    captured: list[httpx.Request] = []
    ...
    async with _make_client(httpx.MockTransport(handler)) as client:
        await client.ingestion_operation.put_item(
            operation_id,
            body={"item_key": "item-1", "memory": {"raw_text": "hello"}},
            collection_id=collection_id,
        )
        await client.ingestion_operation.seal(
            operation_id, collection_id=collection_id
        )
    ...
    for request in captured:
        assert (
            request.headers["x-nebula-owner-key"]
            == f"collection:{collection_id}"
        )
        assert "collection_id" not in request.url.params
    assert "collection_id" not in json.loads(captured[0].content)


def test_explicit_none_routing_value_fails_closed() -> None:
    from nebula._runtime.client import _routing_headers_for_request

    collection_id = "22222222-2222-4222-8222-222222222222"
    body = {"collection_id": collection_id}

    assert _routing_headers_for_request(
        body=body,
        path_params={},
        query=None,
        routing={
    ...
    ) == {"X-Nebula-Owner-Key": f"collection:{collection_id}"}
...
@pytest.mark.asyncio
async def test_wrong_region_misdirect_is_retried_against_converged_directory() -> None:
    attempts = 0
    bodies: list[bytes] = []
        ...
        if attempts == 1:
            return httpx.Response(
                ...
                json={
                    ...
                    "message": "collection is owned by another region",
                    "request_id": None,
                    "details": {
